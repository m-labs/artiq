...
unsafe fn write_byte(mut byte: u8) {
    csr::spiflash_bitstream::bitbang_write(0);
    for _ in 0..8 {
        csr::spiflash_bitstream::bitbang_write((byte & 0x80) >> 7);
        csr::spiflash_bitstream::bitbang_write((byte & 0x80) >> 7 | PIN_CLK);
        byte <<= 1;
    }
    csr::spiflash_bitstream::bitbang_write(0);
}
...
unsafe fn write_addr(mut addr: usize) {
    csr::spiflash_bitstream::bitbang_write(0);
    for _ in 0..24 {
        csr::spiflash_bitstream::bitbang_write(((addr & 0x800000) >> 23) as u8);
        csr::spiflash_bitstream::bitbang_write(((addr & 0x800000) >> 23) as u8 | PIN_CLK);
        addr <<= 1;
    }
    csr::spiflash_bitstream::bitbang_write(0);
}
...
fn wait_until_ready() {
    unsafe {
        loop {
            ...
            for _ in 0..8 {
                sr <<= 1;
                csr::spiflash_bitstream::bitbang_write(PIN_DQ_I | PIN_CLK);
                sr |= csr::spiflash_bitstream::miso_read();
                csr::spiflash_bitstream::bitbang_write(PIN_DQ_I);
            }
            csr::spiflash_bitstream::bitbang_write(0);
            csr::spiflash_bitstream::bitbang_write(PIN_CS_N);
            if sr & SR_WIP == 0 {
                return
            }
        }
...
pub unsafe fn read(mut addr:usize, mut len: u32) -> &[u8] {
    wait_until_ready();
    
    let mut value: [u8; len];
    csr::spiflash_bitstream::bitbang_write(PIN_CS_N);
    write_byte(CMD_RD);
    write_addr(addr);
    for i in 0..len {
