...
DDSParamsSim = namedtuple("DDSParams", ["channels"])

def main():
    w_kasli = iir.IIRWidths(state=25, coeff=18, adc=16, asf=14,
            word=16, accu=48, shift=11, profile=5, dly=8)
    p_adc = ADCParamsSim(channels=8)
    p_dds = DDSParamsSim(channels=12)
    w = iir.IIRWidths(state=17, coeff=16, adc=16, asf=14,
