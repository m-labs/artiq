...
class IIR(Module):
    ...
    """
    def __init__(self, w, adc_channels, dds_channels):
        self.widths = w
        ...
        self.specials.m_coeff = Memory(
                ...
                depth=(4 << w.profile) * dds_channels)
        ...
        self.specials.m_state = Memory(
                ...
                depth=((1 << w.profile) * dds_channels) + (2 * adc_channels))
        ...
        t_current_step = Signal(max=max(adc_channels * 2, dds_channels * 4, ))
        ...
        fsm.act("LOAD",
                self.loading.eq(1),
                If(t_current_step == adc_channels - 1,
                    t_current_step_clr.eq(1),
        ...
        fsm.act("SHIFT",
                self.shifting.eq(1),
                If(t_current_step == (adc_channels * 2) - 1,
                    NextState("IDLE")
                )
        )
        ...
        sel_profile = Signal(max=adc_channels)
        dly_profile = Signal(w.dly)
        assert adc_channels <= 1 << 8
        assert 8 + w.dly <= w.coeff

        # latched adc selection
        sel = Signal(max=adc_channels, reset_less=True)
        # iir enable SR
        ...
        self.comb += [
                ...
                If(self.shifting,
                    m_state.adr.eq(t_current_step + ((1 << w.profile) * dds_channels)),
                    m_state.dat_w.eq(m_state.dat_r),
                    m_state.we.eq(t_current_step[0])
                ),
                If(self.loading,
                    m_state.adr.eq((t_current_step << 1) + ((1 << w.profile) * dds_channels)),
                ...
                If(self.processing,
                    m_state.adr.eq(Array([
                        ...
                        # read x0 (recent)
                        0 | (sel_profile << 1) + ((1 << w.profile) * dds_channels),
                        # read x1 (old)
                        1 | (sel << 1) + ((1 << w.profile) * dds_channels),
                    ])[pipeline_phase]),
    ...
    def set_state(self, channel, val, profile=None, coeff="y1"):
        ...
        if coeff == "y1":
            ...
            yield self.m_state[(channel << 1) |
                    ((1 << w.profile) * self.dds_channels)].eq(val)
        elif coeff == "x1":
            assert profile is None
    ...
    def check_iter(self):
        ...
        data = []
        # predict output
        for i in range(self.dds_channels):
