...
_SDCARD_SPI_CONFIG = (0*SPI_OFFLINE | 0*SPI_END |
...
class CardTest(EnvExperiment):
    ...
    def run(self):
        ...
        for i in range(10):
            self.spi_mmc.write(-1)
        self.spi_mmc.set_config(_SDCARD_SPI_CONFIG | SPI_END, 32, freq, 0)
        self.spi_mmc.write(-1)
        ...
        self.spi_mmc.set_config(_SDCARD_SPI_CONFIG | SPI_INPUT, 8, freq, cs)
        idle = False
        response = 0
        for i in range(8):
            ...
            if response == 0x01:
                idle = True
                break
        self.spi_mmc.set_config(_SDCARD_SPI_CONFIG | SPI_END, 8, freq, cs)
