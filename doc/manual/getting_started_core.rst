...
Note that if processing a kernel statement takes a lot of CPU time on the core device (this is different from the events themselves taking a long time to execute), it may cause a subsequent statement to miss the deadline for timely submission of its events.
