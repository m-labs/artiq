...
RPC functions must always return a value of fixed type. When that value is not ``None``, the compiler needs to be informed in advance of the type of the value, which is what the ``-> TBool`` annotation is for. 
...
For more documentation on the methods used, see the relevant section of the `core drivers reference <https://m-labs.hk/artiq/manual/core_drivers_reference.html#module-artiq.coredevice.dma>`.
