...
It also expects access to a ``device_db.py``, with a corresponding flag ``--device-db`` to substitute a different file name. Additionally, it will reference or create certain files in the directory it is run in, among them ``dataset_db.mdb``, the LMDB database containing persistent datasets, ``last_rid.pyon``, which simply contains the last used RID, and the ``results`` directory. For more on the device and dataset databases, see also :doc:`environment`.
...
The browser *can* connect to the master, specifically in order to be able to access the master's store of datasets and to upload new datasets to it, but alone among the management system components it can also be run completely standalone. It does not require a connected master to run. However, it requires filesystem access to the ``results`` files to be of much use.
...
The use of the Git backend is triggered when the master is started with the ``-g`` flag. Otherwise the bare filesystem is read and Git-based features will not be available.
...
Other flags can also be used, such as ``--class-name`` / ``-c`` to submit by class name, or ``--revision`` / ``-r`` to use a particular revision. See the reference of :mod:`~artiq.frontend.artiq_client` in :doc:`main_frontend_tools`.

In order to run from the raw filesystem when using the dashboard, right-click in the Explorer window and select the option 'Open file outside repository'. This will open a submission window for the experiment. Note however that this feature is not available if the dashboard is run on a different machine to the master, as the dashboard does not submit by content. There is no GUI support for submission by content. It is recommended to simply use the command-line client for this purpose.
