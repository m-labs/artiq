...
* Xilinx Vivado is used to generate the FPGA bit files used by some Sinara hardware (eg Kasli). Download Vivado from Xilinx and install it. Use the same version of Vivado as the M-Labs automated `build server <https://nixbld.m-labs.hk/project/artiq>`_; see `vivado.nix <https://git.m-labs.hk/M-Labs/nix-scripts/src/branch/master/artiq-fast/vivado.nix>`_ to determine the verison. There are a variety of pitfalls associated with installing Vivado; consult the `forum <https://forum.m-labs.hk/?q=vivado>`_ for advice from the user community. 
