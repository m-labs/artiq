...
* Enter the development shell by running ``nix develop github:m-labs/artiq``, or alternatively by cloning the ARTIQ Git repository and running ``nix develop`` at the root (where ``flake.nix`` is).
