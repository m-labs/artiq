...
To communicate securely, the master must have access to its own key and certificate as well as the client certificate (to be able to identify legitimate clients). Place these files in the same directory you run the master from. To enable encryption, use the ``--ssl`` flag: ::
