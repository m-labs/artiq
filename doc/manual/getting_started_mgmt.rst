...
.. note::
    The ``--content`` flag submits by content, that is, from the client's local environment, instead of from the master's. Since you are likely running the client and the master on the same machine, it is probably not strictly necessary here. You can also right-click in the Explorer dock and select the 'Open file outside repository' option for the same effect. In a distributed arrangement across multiple machines, ``--content`` must be used and the GUI option will not be available.
